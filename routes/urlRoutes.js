...
router.get('/getOne/:dataID', (req, res) => {
    try{
        url.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
                return res.status(400).send({message: 'Error while retrieving an url. Please check the data'})
            }
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        url.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        url.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
